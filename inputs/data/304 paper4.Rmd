...
geom_bar(stat='identity',position = "dodge")+
  labs(title  = "Graph 3: Mean percent by residence", x="Residence", y="Mean value in percentage")+theme( plot.title=element_text(size=15), axis.text.x=element_text(size=10,angle=45, hjust=1))+scale_fill_brewer(palette="green")

```
The bar plot shows the percent distribution of the male household population by three different levels of education attended according to different residence types. The x-axis shows eight main residences in Tanzania and the total male population is classified based on their highest education level. In other words, we group male population according to: people who had no education experience at all, have completed primary school, or had attended secondary or higher education schools. The y-axis shows their corresponding value of each residence. According to the graph, we can see that the mean percentage of male population that had completed primary education has the highest mean
...
### 4.3.1 Factors affect median year of schooling 
...
data_region_for_regress = data0%>%
  slice(24:43)%>%
  select(median_year_of_schooling, background_characteristic,no_education,completed_primary,some_secondary_and_higher)
model <- lm(median_year_of_schooling ~ completed_primary+some_secondary_and_higher, data = data_region_for_regress)
summary(model)
```
